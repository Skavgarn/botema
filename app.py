...
def place_order(symbol, typ):
    if (typ == "BUY"):
        try:
            ...
            QUAN = round_down(QUAN, len(get_tick_and_step_size(symbol).split(".")[1]));
            print(QUAN)
        ...
        try:
            print(client.futures_create_order(symbol=symbol, quantity=QUAN, type="MARKET", side=typ,
                                              reduceOnly=True));  # RISK_AMOUNT2 += (RISK_AMOUNT2/100) * CHANGE_IN_AMOUNT
        except Exception as e:
...
def main():
    ...
    print("Watching ", symbol)  # print("buy",buy)"""
    while True:
        data = get_data()
